...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($kode)) {
        die("Kode pemulihan tidak boleh kosong.");
    }

    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

    $stmtUser = $conn->prepare("INSERT INTO user (nama, email, password, role) VALUES (?, ?, ?, ?)");
    $stmtUser->bind_param("ssss", $nama, $email, $hashedPassword, $role);
    $stmtUser->execute();

    $idUser = $conn->insert_id;

    $stmtReset = $conn->prepare("INSERT INTO reset_password (kode_reset, id_user) VALUES (?, ?)");
    $stmtReset->bind_param("si", $kode, $idUser);
    $stmtReset->execute();
...
<body>
    ...
    <div class="container-register">
        <form id="registerForm" class="login-form needs-validation" method="POST" action="" novalidate>
            <h4 class="mb-4 text-center fw-bold">Register</h4>

            <!-- Nama -->
            <div class="mb-3">
                <label for="registerName" class="form-label">Nama</label>
                <input type="text" class="form-control" name="nama" id="registerName" placeholder="Masukkan nama" required>
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control" name="email" id="registerEmail" placeholder="Masukkan email" required>
            ...
            <div class="mb-3">
                ...
                <div class="position-relative">
                    <input type="password" class="form-control" name="password" id="registerPassword" placeholder="Masukkan password" required minlength="6">
            ...
            <div class="mb-3">
                <label for="kodeUnik" class="form-label">Kode Pemulihan <small class="text-muted d-block mt-1">Gunakan kode ini saat ingin mereset password Anda.</small></label>
                <input type="text" class="form-control" name="kode" id="kodePemulihan" placeholder="Masukkan kode pemulihan" required>
                <div class="invalid-feedback">Harap masukkan kode pemulihan Anda.</div>
            </div>

            <!-- Error login -->
            <div id="loginError" class="text-danger mb-3 d-none">
                Email atau password salah.
            </div>

            <!-- Tombol Login -->
            <button type="submit" name="submit" class="btn btn-primary w-100 mt-2">Register</button>

            <!-- Daftar -->
            <div class="mt-3 text-center">
                <span>Sudah punya akun? <a href="login-user.php" class="no-blue-link ms-2">Login</a></span>
            </div>
        </form>
